...
const getS3Client = () =>
  new S3Client({
    ...
    endpoint: process.env.DOWNLOAD_STORAGE_ENDPOINT,
    forcePathStyle: process.env.DOWNLOAD_STORAGE_FORCE_PATH_STYLE === "true",
    credentials: {
...
export const createSignedProductDownloadUrl = async (product: Product) => {
  ...
  const command = new GetObjectCommand({
    ...
    ResponseContentDisposition: `attachment; filename="${getDownloadFilename(product)}"`,
  });
